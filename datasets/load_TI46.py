...
def load_and_process_ti46(
    ...
    (train_signal, train_label, train_rate, _,) = load_TI20(speakers, train=False)
    test_signal, test_label, _, _ = load_TI20(speakers)

    # Apply a audio signal preprocessing step called MFCC
    pre_process = audio_preprocess.mfcc(rate=train_rate[0])
    x_train, x_test = (
        pre_process.fit_transform(train_signal),
        pre_process.fit_transform(test_signal),
    )
